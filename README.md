...
once) and the **TouchDesigner Parameter Rack** (parameters out, an audio level and a sensor back in, with the two-CHOP recipe on its card; it wears the Sci-fi HUD style rather than a look of its own). **Big Thumb** puts one control on each screen of a phone, the size of a
