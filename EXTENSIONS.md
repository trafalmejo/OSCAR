...
| `surfaces` | the published surfaces: `list()`, `widgets(id)`, and `drive(id, widgetId, state)`, which puts one widget in one state. The caller never says where that goes: the destination, encoding and range come from the surface as published. Use this, not raw OSC, for anything acting on a rig: a schedule, or an instruction from somewhere untrusted. `snapshot()` and `onState(fn)` follow what every control is showing, whoever changed it |
