...
test("the OSC section is built in one place, for the directions a widget has", () => {
  ...
  assert.deepStrictEqual(keys(), ["listen", "oscListenFrom", "oscEnabled", "oscSendWhen", "oscLoopGuard", "oscVia", "ip", "port", "message"]);
  assert.deepStrictEqual(keys({ sends: false }), ["listen", "oscListenFrom", "message"], "a widget that only follows has nowhere to send");
