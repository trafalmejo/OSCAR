...
/**
 * The widgets that can drive a DMX channel.
 *
 * Not all of them can, and that is deliberate rather than an omission: a DMX
 * channel carries a level, so a widget has to produce one. A meter only ever
 * displays, and a text box sends words. Both are excluded on purpose, and the
 * test below says so out loud.
 */
const DMX_CAPABLE = WIDGETS.filter((w) => w.fields.some((f) => f.key === "transport"));
...
test("a widget left on OSC puts nothing on the DMX wire", () => {
  ...
  for (const widget of DMX_CAPABLE) {
...
test("every widget that can drive DMX does it in the same words", () => {
  for (const widget of DMX_CAPABLE) {
...
test("a widget with no level to give is deliberately kept off DMX", () => {
  // Stated as a test so that adding a widget makes someone decide, rather than
  // letting a control quietly acquire a DMX panel it cannot honour.
  const names = WIDGETS.filter((w) => DMX_CAPABLE.indexOf(w) === -1).map((w) => w.name);

  assert.ok(names.includes("oscar-meter"), "a meter reads, it never drives");
  assert.ok(names.includes("oscar-text"), "a channel takes a level, not words");

  for (const widget of WIDGETS) {
    if (DMX_CAPABLE.indexOf(widget) !== -1) continue;
    const keys = widget.fields.map((f) => f.key);
    for (const dmxKey of ["dmxUniverse", "dmxChannel", "dmxCount"]) {
      assert.ok(!keys.includes(dmxKey), widget.name + " should not offer " + dmxKey);
    }
  }
});
...
// --- the display widgets follow the network too ------------------------------

test("a listening meter moves with incoming OSC and sends nothing back", () => {
  const { el, ctx } = mount(meter, { listen: true, min: 0, max: 100 });
  ctx.receive(meter.defaults.message, [75]);

  assert.strictEqual(el.style.properties["--oscar-level"], "75.00%");
  assert.deepStrictEqual(ctx.sent, [], "a meter never sends");
});

test("an unreadable level leaves the meter where it was", () => {
  // Falling to zero would report silence on a channel that may be at full,
  // which is the dangerous direction for a display to fail in.
  const { el, ctx } = mount(meter, { listen: true, min: 0, max: 100 });
  ctx.receive(meter.defaults.message, [60]);
  const held = el.style.properties["--oscar-level"];

  for (const bad of ["", null, "abc"]) {
    ctx.receive(meter.defaults.message, [bad]);
    assert.strictEqual(el.style.properties["--oscar-level"], held, JSON.stringify(bad));
  }
});

test("a meter with Listen off stays still", () => {
  const { el, ctx } = mount(meter, { min: 0, max: 100 });
  ctx.receive(meter.defaults.message, [75]);
  assert.notStrictEqual(el.style.properties["--oscar-level"], "75.00%");
});

test("the grid follows the clip the software says is playing", () => {
  const { ctx } = mount(media, {
    listen: true,
    items: "Clip 1|1; Clip 2|2; Clip 3|3",
  });

  ctx.receive(media.defaults.message, [3]);
  assert.strictEqual(ctx.config.selected, 3, "highlight moved to the third tile");
  assert.deepStrictEqual(ctx.sent, [], "following a clip must not relaunch it");
});

test("a clip the grid does not have leaves the selection alone", () => {
  const { ctx } = mount(media, {
    listen: true,
    items: "Clip 1|1; Clip 2|2",
    selected: 1,
  });

  ctx.receive(media.defaults.message, [99]);
  assert.strictEqual(ctx.config.selected, 1);
});
