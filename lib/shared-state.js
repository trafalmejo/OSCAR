...
// invents ids would otherwise fill memory one record at a time. Past this
// many, the record written longest ago makes room for the new one (see
// apply): refusing the newcomer instead would let one page inventing ids
// lock every real widget not yet recorded out of the store for the rest of
// the show, while a real widget that is pushed out is back the next time
// anyone touches it.
...
// snapshot() is a plain object keyed by id, and so is what JSON makes of it
// in the browser. On such an object "__proto__" is not a key but the
// prototype: the record would be broadcast live yet missing from every
// snapshot, and would swap the prototype of the browser's cache. No real
// component id looks like this, so it is refused at the door.
function isId(id) {
...
class SharedState {
  ...
  /**
   * Be told of every change, whoever made it: fn(id, state), the widget's
   * whole state after the change. For something that passes the surface's
   * state on somewhere else. Returns an unsubscribe function. A watcher that
   * throws is dropped from that one call, never from the record.
   */
  onChange(fn) {
    if (typeof fn !== "function") return function () {};
    this.watchers.push(fn);
    return () => {
      this.watchers = this.watchers.filter((other) => other !== fn);
    };
  }
   ...
   *
   * @param {{heard?: boolean}} [info] passed on to whoever is watching
  ...
  apply(id, patch, info) {
    ...
    // A Map keeps insertion order, so deleting before setting moves this
    // record to the back and leaves the one written longest ago at the
    // front, which is the one to go when the store is full.
    this.byId.delete(id);
    if (this.byId.size >= MAX_WIDGETS) this.byId.delete(this.byId.keys().next().value);
    this.byId.set(id, next);
    for (const fn of this.watchers.slice()) {
      try {
        // `info` says where it came from, for a watcher that treats a stream
        // from the rig ({ heard: true }) differently from a hand.
        fn(id, Object.assign({}, next), info || {});
      } catch (err) {
