 ...
 * @param {(dir: "in"|"out") => void} [deps.onActivity] told when the server works for a
 *        published surface: "in" as it consumes OSC or MIDI for one, "out" as it sends
 *        on one's behalf. What the editor's LIVE pill flickers on.
 */
function createSurfaces(deps) {
  ...
  return {
    ...
    async drive(surfaceId, widgetId, state, options) {
      ...
      // pass through here.
      if (told && deps.onActivity) deps.onActivity("out");
    ...
    async hearOsc(message) {
      ...
      if (moved && deps.onActivity) deps.onActivity("in");
      return moved;
    },
