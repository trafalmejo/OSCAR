  ...
  return {
    ...
    async drive(surfaceId, widgetId, state, options) {
      ...
      for (const message of messages) {
        if (message.address && silent.indexOf("osc") === -1) deps.sendOSC(message.ip, message.port, message.address, message.args);
        if (message.dmx && silent.indexOf("dmx") === -1) deps.sendDMX(Object.assign({ source: target.id }, message.dmx));
