...
function createSurfaces(deps) {
  ...
  /** Record what the rig put a widget in. Nobody on the network is told: every page there heard it too. */
  function record(id, state) {
    if (!deps.shared || !deps.shared.store) return false;
    return deps.shared.store.apply(id, state, { heard: true }) !== null;
  }
  ...
  return {
    ...
    /**
     * An OSC message arrived. The pages on the network hear it for
     * themselves; this is for whoever cannot, a phone across the internet,
     * and for a venue with no page open at all. Each published widget that
     * follows that address, and can say what the message does to it
     * (`hear` in its definition), has the state recorded. Nothing is sent
     * and nobody on the network is told.
     *
    ...
    async hearOsc(message) {
      ...
      for (const entry of await allWidgets()) {
        ...
        const match = incoming(widget.config, message);
        if (!match) continue;
