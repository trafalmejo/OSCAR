...
const { isListening, midiIndex } = require("./midi/spec");
...
function createSurfaces(deps) {
  ...
  return {
    ...
    /** The input ports those widgets name, for the MIDI input to open: parts of names, "" for every port. */
    async midiPorts() {
      const parts = (await this.midiListeners()).map((entry) => String(entry.widget.config.midiInPort || "").trim());
     ...
     */
    async hearMidi(heard, port) {
      let moved = 0;
      const showing = this.snapshot();
      for (const entry of await this.midiListeners()) {
        const widget = entry.widget;
        const index = midiIndex(widget.config, heard, port, valuesOf(widget.definition, widget.config));
        if (index === -1) continue;
