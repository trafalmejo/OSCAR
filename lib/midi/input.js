 ...
 * @param {(heard: object, port: string, first: boolean) => void} options.onMessage a note, controller, program or bend, as readMidi() says it; `first` when it came in by the first port there is
...
function createMidiInput(driver, options) {
  ...
  let wanted = []; // parts of names, lower case; "*" wants every port, "" the first
  let first = null; // the first port there is, as of the last look
  let timer = null;
  let heard = 0;
  let error = null;
  ...
  function wants(name) {
    ...
    return wanted.some((part) => (part === "*" ? true : part === "" ? name === first : lower.indexOf(part) !== -1));
  }
  ...
  function check() {
    ...
    first = names.length ? names[0] : null;
    ...
    for (const name of names) {
      ...
      try {
        const port = driver.openInput(name, function (bytes) {
          ...
          if (!message) return;
          heard++;
          opts.onMessage(message, name, name === first);
        });
        if (!port) continue;
        open.set(name, port);
        error = null;
        ...
        const unsaid = error === null;
        error = name + ": " + String((err && err.message) || err);
        if (unsaid && opts.onError) opts.onError(new Error(error));
      }
    }
  }
   ...
   * every port, "" for the first there is, [] for none. Replaces what was
   * wanted before.
   */
  function want(parts) {
