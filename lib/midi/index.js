...
function createMidi(options) {
  ...
  const input = createMidiInput(driver, {
    ...
    onMessage: function (heard, port) {
      ...
      for (const fn of listeners.slice()) fn(heard, port);
    },
  });
  ...
  /** Say which input ports the widgets want ("" for every port, [] for none). Be told of what arrives with onMessage(). */
  function listenFor(parts) {
    bridged = (parts || []).slice();
    rewant();
  }

  /** fn(heard, port) for every note, controller, program and bend. Returns an unsubscribe function. */
