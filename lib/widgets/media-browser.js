...
const { ARG_TYPES, toNumber } = require("../osc-args");
 ...
 * pieces the split made of it are put back together.
 *
...
function parseItems(raw) {
  ...
  for (let i = 0; i < pieces.length; i++) {
    let piece = pieces[i];
    while (unfinishedDataUrl(piece) && i + 1 < pieces.length) piece += ";" + pieces[++i];
...
function safeImageUrl(raw) {
  ...
  if (name === "data" && /^data:image\//i.test(bare)) return url;
  return "";
}
...
const mediaBrowser = {
  ...
  attach: function (el, ctx) {
    ...
    function pick(item) {
      ...
      selected = item.value;
      paint();
      ctx.send(outgoing(routing(ctx), item.value));
 ...
 * apart. And an image URL that will be refused is said here, where the
...
function checkItems(raw, config) {
  ...
  for (const item of items) {
    ...
    if (seen["=" + item.value]) return 'Two items send "' + item.value + '"; each item needs its own value';
