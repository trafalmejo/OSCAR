 ...
 *   share(state)             tell every other device showing this surface
 ...
 *                            OSC receive path, so the record on the server
 *                            has what the rig said. Refused while a shared
 *                            state is being delivered (see onShared).
 *                            Optional in a host: absent where there is one
 *                            device and nothing to agree with, so widgets go
 *                            through share() in shared.js, which checks.
 ...
 * hand on it outranks the other devices as it outranks the rig.
 *
