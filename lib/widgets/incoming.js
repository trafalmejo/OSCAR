 ...
 * literally; the incoming address is the pattern (lib/osc-address.js).
 *
...
function incoming(config, message) {
  ...
  for (const address of addresses) {
    if (matchesAddress(message.address, address)) {
 ...
 *
 * Listen and Message are read afresh for every message, because both can be
 * edited while the widget is live. `addresses`, if given, is a function
 * returning the address or addresses to follow instead of Message.
 *
...
function follow(ctx, fn, addresses) {
  ...
  return ctx.onOsc(function (message) {
    ...
    const match = incoming({ listen: ctx.get("listen"), message: wanted }, message);
