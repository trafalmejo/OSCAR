 ...
 * are never a hex code; a string is only taken as one when the list does not
 * read as channels, which is always so for the single string OSCAR itself
 * sends. The rule lives here and not in parseHex because a bare "100" typed
...
function fromWire(values, scale) {
  ...
  return typeof values[0] === "string" ? normaliseHex(values[0]) : null;
}
