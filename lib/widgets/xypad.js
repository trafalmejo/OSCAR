...
const xypad = {
  ...
  attach: function (el, ctx) {
     ...
     * passes the position on to the other devices once: they heard the rig
     * too, so the server finds nothing changed in what they say back, and a
     * device joining later starts where the rig left the handle.
     */
    function adopt(values, address) {
      ...
      place();
      share(ctx, { x: ctx.get("x"), y: ctx.get("y") });
    }
