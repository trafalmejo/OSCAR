...
const { canvasStylesheets, SURFACE_CSS } = require("../widget-styles");
...
function sources(publicDir) {
  return {
    ...
    // Everything the canvas loads to draw a widget: each style's tokens, then
    // the widget rules. The bundled fonts are left out. Their stylesheets
    // point at dozens of font files by relative path, and a style's font
    // token ends in a system font, which is what the page falls back to.
    widgetCss: canvasStylesheets()
      .filter((ref) => !ref.startsWith("node_modules/"))
      .map((ref) => path.join(publicDir, ref)),
  };
}
...
function buildExport(body, deps) {
  ...
  const widgetCss = [];
  for (const file of [].concat(files.widgetCss || [])) {
    try {
      widgetCss.push(fs.readFileSync(file, "utf8"));
    } catch (err) {
      // Unstyled controls still send; worth carrying on for.
    }
  }
  // The surface's own background, text colour and font follow its style too.
  // Last, and outside any layer, for the reason given where it is defined.
  widgetCss.push(SURFACE_CSS);
