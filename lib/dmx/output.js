...
const { SLOTS, protocol, defaultHost, isUsb } = require("./spec");
...
/**
 * @param {function} send  (packet, port, host, target) => Promise | undefined; the only
 *                         way out. `target` is { protocol, host, port, universe }: a USB
 *                         protocol's packet is the bare 512-level frame, for a serial port
...
function createDmxOutput(send, options) {
  ...
  function encode(stream, terminated) {
    // A USB interface takes the frame itself; the driver frames it for the wire.
    if (isUsb(stream.protocol)) return Buffer.from(stream.frame);
  ...
  function transmit(stream, terminated) {
    ...
    try {
      return Promise.resolve(send(encode(stream, !!terminated), stream.port, stream.host, describe(stream))).catch(onError);
    } catch (err) {
      onError(err);
