...
/**
 * How long a list of the serial ports is trusted. Listing is not cheap: on
 * Linux serialport runs `udevadm info -e`, which takes seconds on a small
 * board, and asking for every frame piled those up by the hundred on a
 * Raspberry Pi 1 until no frame got through at all.
 */
...
function createUsbDmx(deps) {
  ...
  // The last list of ports, and the one being made. One listing at a time,
  // shared by every frame that asks while it runs, and kept for LIST_TTL_MS.
  let listed = null;
  let listing = null;

  function listPorts() {
    if (listed && Date.now() - listed.at < LIST_TTL_MS) return Promise.resolve(listed.ports);
    if (!listing) {
  ...
  /** Something changed on the wire -- a port failed or went away: list again next time. */
  function forgetPorts() {
    listed = null;
  }
  ...
  function runLoop(line, path) {
    ...
    const tick = () => {
      ...
      // The clock's own now, never Date.now(): under a test's fake clock,
      // real milliseconds leaking in here decided whether the frame's next
      // tick landed inside or outside the test's window -- the flake.
      const started = clock.now ? clock.now() : Date.now();
      openDmxFrame(line, path).then(() => {
        if (!lines.has(path) || line.stopped) {
          line.loop = null;
          return;
        }
        const elapsed = (clock.now ? clock.now() : Date.now()) - started;
        line.loop = clock.setTimeout(tick, Math.max(1, OPEN_DMX_FRAME_MS - elapsed));
      });
    };
...
module.exports = { createUsbDmx, pickPort, proPacket, openDmxPacket, portSettings, KINDS, OPEN_RETRY_MS, OPEN_DMX_FRAME_MS, LIST_TTL_MS, FTDI_VENDOR };
