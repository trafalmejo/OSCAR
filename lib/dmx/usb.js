...
function createUsbDmx(deps) {
  ...
  /** Keep an Open DMX line refreshed at forty frames a second while it is driven. */
  function runLoop(line, path) {
    ...
    const tick = () => {
      ...
      openDmxFrame(line, path).then(() => {
        if (lines.has(path) && !line.stopped) line.loop = clock.setTimeout(tick, OPEN_DMX_FRAME_MS);
        else line.loop = null;
      });
    };
