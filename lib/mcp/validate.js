...
function validateSurface(html) {
  ...
  const loose = parts.filter((p) => p.tag !== "script" && p.tag !== "style" && p.attrs["data-gjs-dmode"] !== "flow").length;
  if (loose) {
    warnings.push(
      loose +
        ' element(s) have no data-gjs-dmode="flow". The editor drags in absolute mode, and a part without it is pulled out of the layout when touched -- scaling moves it instead. create_surface adds the attribute where it is missing.'
    );
  }
...
/**
 * Stamp data-gjs-dmode="flow" on every element in <body> that lacks it, the
 * way OSCAR's own templates do (test/templates.test.js holds them to it):
 * the editor drags in absolute mode, and an unstamped part is pulled out of
 * the layout the moment a handle touches it -- scale moves it, and
 * overlapping parts grab each other. Script and style bodies are left
 * strictly alone: markup inside a string is code, not a tag.
 */
function stampFlow(html) {
  const text = String(html);
  const at = text.search(/<body\b/i);
  if (at === -1) return text;
  const head = text.slice(0, at);
  const pieces = text.slice(at).split(/(<script\b[\s\S]*?<\/script>|<style\b[\s\S]*?<\/style>)/gi);
  const stamped = pieces.map((piece, i) => {
    if (i % 2 === 1) return piece; // a whole script or style element, captured by the split
    return piece.replace(/<([a-z][a-z0-9]*)\b([^>]*?)(\s*\/)?>/gi, (tag, name, attrs, slash) => {
      if (/^(body|script|style|br|hr|meta|link|title)$/i.test(name)) return tag;
      if (/\bdata-gjs-dmode=/i.test(attrs)) return tag;
      return "<" + name + attrs + ' data-gjs-dmode="flow"' + (slash ? " /" : "") + ">";
    });
  });
  return head + stamped.join("");
}

module.exports = { validateSurface, stampFlow, bodyTags, settingsOf, widgetFor };
