...
function dressServer(tools, version, onToolCall) {
  ...
  for (const tool of tools) {
    server.registerTool(
      ...
      async (args) => {
        // Told by name only (telemetry, anonymous): which tools earn their keep.
        if (onToolCall) onToolCall(tool.name);
 ...
 */
function attachMcp(app, { tools, version, token, enabled, onToolCall }) {
  ...
  app.post("/mcp", async (req, res) => {
    ...
    try {
      const server = dressServer(tools, version, onToolCall);
